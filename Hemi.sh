...
view_logs() {
    ...
    if [ ! -d "$DIRECTORY" ]; then
        ...
        echo -e "${CYAN}Please ensure the node is installed correctly.${NC}"
        echo -e "${CYAN}Press any key to return to the main menu...${NC}"
        read -n 1 -s
        return
    fi

    echo -e "${GREEN}Entering directory:${NC} $DIRECTORY"
    cd "$DIRECTORY" || { echo -e "${RED}Failed to enter directory.${NC}"; return; }

    # Check if pm2 is installed
    if ! command -v pm2 &> /dev/null; then
        echo -e "${RED}Error: PM2 is not installed. Logs cannot be viewed.${NC}"
        echo -e "${CYAN}Please install PM2 and try again.${NC}"
        echo -e "${CYAN}Press any key to return to the main menu...${NC}"
        read -n 1 -s
        return
    fi
    ...
    if ! pm2 list | grep -q "popmd"; then
        echo -e "${RED}Error: popmd process is not running in PM2.${NC}"
        echo -e "${CYAN}Please start the node and try again.${NC}"
        echo -e "${CYAN}Press any key to return to the main menu...${NC}"
        read -n 1 -s
        return
    fi

    echo -e "${YELLOW}Displaying PM2 logs for popmd...${NC}"
    pm2 logs popmd
